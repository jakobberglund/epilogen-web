<article>
  <header>
    <nav>
      <a href="#biografi">Biografi,</a>
      <a href="#lankar">Länkar,</a>
      <a href="#bilder">Bilder,</a>
      <a href="#recensioner">Recensioner</a>
      <a href="/en/press">In English</a>
    </nav>
  </header>
  <h1>Press</h1>
  <section>
    <h2 id="biografi">Biografi</h2>
    <p>
      Den 14 april släpper Epilogen sitt tredje album, Nollpunkten. Det är på många sätt en
      syskonskiva till Dekonstruktion, bandets andra album som släpptes i december förra året, men
      ändå med en tydlig egen identitet. På Nollpunkten arbetar Epilogen med att skapa en egen
      ljudvärld, där de långa låtarna tar avstamp i skenbart enkla strukturer där atmosfärer
      kompromisslöst skulpteras fram med ett anammande av tillfälligheter och improvisation. Det är
      en skiva som växer och förändras med varje lyssning. Musikaliskt är det en mörk blandning av
      rock, elektronik och rostiga analoga industriella ljudlandskap.
    </p>
    <p>
      Epilogen är Stockholmsduon Jakob Berglund på sång, synthar och maskiner, och Anders Carlström
      på gitarrer och bas. Anders och Jakob är vänner sedan barndomen, och har länge spelat musik,
      var för sig och tillsammans. Jakob är medlem i A Swarm of the Sun, där även Anders spelar bas
      live och på skiva. Anders spelar även i The Isolation Process och Kausal.
    </p>
    <p>Nollpunkten är producerad och mixad av Jakob Berglund.</p>
  </section>
  <section>
    <h2 id="lankar">Länkar</h2>
    <p>
      <a href="https://epilogen.bandcamp.com/album/nollpunkten">Bandcamp</a>
      <a href="https://open.spotify.com/artist/6D2X3gxJG0yo8MiE2VtmvH?si=-mcydiRdTlaKT-z1GIoF3w"
        >Spotify</a
      >
      <a href="https://music.apple.com/us/artist/epilogen/1592826022">Apple Music</a>
      <a href="https://www.instagram.com/epilogenmusik/">Instagram</a>
      <a href="https://www.facebook.com/epilogenmusik">Facebook</a>
      <a href="mailto:epilogenmusik@gmail.com">Maila oss</a>
    </p>
  </section>
  <section>
    <h2 id="bilder">Bilder</h2>
    <h3>Nollpunkten - omslagsbild</h3>
    <p>
      Omslaget är designat av Epilogen. Det är ett skuret kollage av analogt, egenframkallat
      fotografi.
    </p>
    <figure>
      <img
        src="/epilogen-nollpunkten-artwork-1200x1200.jpg"
        width="1200"
        height="1200"
        loading="lazy"
        alt="Nollpunkten artwork"
      />
    </figure>
    <p>
      Ladda hem
      <a href="/epilogen-nollpunkten-artwork-3000x3000.jpg" target="_blank">3000x3000</a>
      <a href="/epilogen-nollpunkten-artwork-1200x1200.jpg" target="_blank">1200x1200</a>
    </p>
    <h3>Bandfoton</h3>
    <p>Foto: Johan Söderling och Jakob Berglund, 2022</p>
    <figure>
      <img
        src="/epilogen-bandfoto-1.jpg"
        width="3000"
        height="2000"
        loading="lazy"
        alt="Epilogen bandfoto"
      />
    </figure>
    <p>
      Ladda hem
      <a href="/epilogen-bandfoto-1.jpg" target="_blank">3000x2000</a>
    </p>
    <figure>
      <img
        src="/epilogen-bandfoto-2.jpg"
        width="3000"
        height="2000"
        loading="lazy"
        alt="Epilogen bandfoto"
      />
    </figure>
    <p>
      Ladda hem
      <a href="/epilogen-bandfoto-2.jpg" target="_blank">3000x2000</a>
    </p>
  </section>
  <section>
    <h2 id="recensioner">Recensioner</h2>
    <p>
      <a
        href="https://gaffa.se/recensioner/2022/december/album/epilogen-dekonstruktion-en-omgivande-nastan-fysisk-kansla-av-overgivenhet/"
        >Gaffa om Dekonstruktion:</a
      ><br />
      "En omgivande, nästan fysisk känsla av övergivenhet."<br />Betyg 5 av 6
    </p>
    <p>
      <a href="https://www.opulens.se/musik/kangas-klanger-skonhet-mystik-och-en-riktig-parla/"
        >Opulens om Dekonstruktion:</a
      ><br />
      "Söndersprängd men sammansvetsad svävar och knarrar musiken sakta fram för att sedan brista ut
      i kreativa krevader där suset från avgrunden gör sig påmint."
    </p>
    <p>
      <a href="https://482mhz.com/2022/12/02/recensioner-2022-12-02/">482mhz om Dekonstruktion:</a
      ><br />
      "De vill bevisligen krossa oss, men det är för njutningsfullt för att man ska kunna bli krossad."<br
      />Betyg 7 av 10
    </p>
    <p>
      <a href="https://482mhz.com/2021/11/26/recensioner-2021-11-26/">482mhz om Den tysta massan:</a
      ><br />
      "Mitt nya svenska favoritband heter Epilogen."<br />Betyg 8 av 10
    </p>
  </section>
</article>

<style>
  article {
    display: flex;
    flex-direction: column;
    align-items: center;
    padding: 2rem;
    min-height: 100vh;
  }

  section {
    width: 100%;
    max-width: 700px;
    text-align: start;
  }

  figure img {
    border: 1px solid #222;
    margin-bottom: 1em;
  }

  p:not(:last-child) {
    margin-bottom: 1em;
  }

  img {
    width: 100%;
    height: auto;
  }

  a {
    color: #5aaf9b;
  }

  h3 {
    margin-bottom: 0.5em;
  }
</style>
